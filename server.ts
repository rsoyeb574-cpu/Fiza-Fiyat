...
async function startServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      if (!apiKey) {
        const fallbackText = "Fiza Hayat specializes in end-to-end luxury building design, Revit BIM modeling, photorealistic 8K rendering, and AI concept generation. Would you like to view our latest portfolio or generate an instant cost estimate?";
        return res.json({ 
          text: fallbackText,
          reply: fallbackText,
          status: 'fallback'
      ...
      const generatedText = response.text || "Fiza Hayat provides world-class architectural and BIM modeling services.";
      res.json({ text: generatedText, reply: generatedText, status: 'success' });
    } catch (error) {
      console.error('Gemini API error:', error);
      const fallbackText = "Fiza Hayat specializes in end-to-end luxury building design, Revit BIM modeling, photorealistic 8K rendering, and AI concept generation.";
      res.json({ 
        text: fallbackText,
        reply: fallbackText,
        status: 'error'
...
Return a structured JSON with:
{
  ...
  "estimatedCostImpact": "1 sentence cost impact"
      ...
      if (jsonMatch) {
        ...
        return res.json({ ...parsed, status: 'success' });
      }
      return res.status(200).json({ status: 'fallback' });
    } catch (error) {
